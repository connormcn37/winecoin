...
const char * const BITCOIN_CONF_FILENAME = "wine.conf";
const char * const BITCOIN_PID_FILENAME = "wined.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "wine" is a composite category enabling all Wine-related debug output
            if(ptrCategory->count(string("wine"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WineCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WineCore
    // Mac: ~/Library/Application Support/WineCore
    // Unix: ~/.winecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WineCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WineCore";
#else
    // Unix
    return pathRet / ".winecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty wine.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override wine.conf
